...
const CookiePolicy: React.FC<LegalPageProps> = ({ onClose }) => {
    const { t } = useContext(LanguageContext);
    return (
        <>
            <div className="flex justify-between items-center p-6 border-b no-print">
                <h3 className="text-xl font-bold text-primary-dark">{t('legal.cookie_policy.title')}</h3>
            ...
            <div className="p-6 overflow-y-auto custom-scroll flex-grow">
                <pre className="whitespace-pre-wrap font-sans text-neutral-700">{t('legal.cookie_policy.content')}</pre>
            </div>
